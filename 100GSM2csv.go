...
func main() {
    ...
    // var mostGSM [][]int
    most := 100
    mostGSM := make([][]int, len(files))
    for i := 0; i < len(mostGSM); i++ {
        mostGSM[i] = make([]int, most)
    }
    for k := range gsm {
        if gsm[k] != -1 {
            insert(mostGSM, gsm[k], k, gsmFreq)   
        }
    }

    // Merge the unique GSM from genome files & reads to csv file
    fmt.Println(len(mostGSM), len(mostGSM[0]))
    for i := 0; i < len(mostGSM); i++ {
...
func insert(a [][]int, index int, k int, gsmFreq map[int]int) {
    if gsmFreq[k] > gsmFreq[a[index-1][len(a[0])-1]] {
        a[index-1][len(a[0])-1] = k
        for i := len(a[0])-1; i > 0 ; i-- {
            if gsmFreq[a[index-1][i]] > gsmFreq[a[index-1][i-1]] {
                a[index-1][i], a[index-1][i-1] = a[index-1][i-1], a[index-1][i]
            }
        }
    }
}
